...
def calc_embeddings(s2v_similarity, spectra):
    ...
    spectral_embeddings = [np.array(s2v_similarity._calculate_embedding(Mass2MotifDocument(spectrum))) for spectrum in spectra]
...
def get_library_matches(similarities, indices, db_path, top_n=10, unique_mols=True):
    ...
    library_matches = [get_library_matches_per_motif(similarities, indices, db_path, motif_number=i, top_n=top_n, unique_mols=unique_mols) for i in tqdm(range(num_motifs))]
...
if __name__ == "__main__":
    ...
    spectrum_1 = Spectrum(mz=np.array([100, 150, 200.]), intensities=np.array([0.7, 0.2, 0.1]), metadata={'id': 'spectrum1', 'precursor_mz': 201.})
    spectrum_2 = Spectrum(mz=np.array([100, 140, 190.]), intensities=np.array([0.4, 0.2, 0.1]), metadata={'id': 'spectrum2', 'precursor_mz': 233.})
    spectrum_3 = Spectrum(mz=np.array([110, 140, 195.]), intensities=np.array([0.6, 0.2, 0.1]), metadata={'id': 'spectrum3', 'precursor_mz': 214.})
    spectrum_4 = Spectrum(mz=np.array([100, 150, 200.]), intensities=np.array([0.6, 0.1, 0.6]), metadata={'id': 'spectrum4', 'precursor_mz': 265.})
    motif_spectra = [add_losses(spectrum_1), add_losses(spectrum_2), add_losses(spectrum_3), add_losses(spectrum_4)]

    # Load data
    s2v_similarity = load_s2v_model(r"C:\Users\dietr004\Documents\PhD\computational mass spectrometry\WP1\MS2LDA\MS2LDA\Add_On\Spec2Vec\model_positive_mode\020724_Spec2Vec_pos_CleanedLibraries.model"
)
    
    embeddings = np.load(r"C:\Users\dietr004\Documents\PhD\computational mass spectrometry\WP1\MS2LDA\MS2LDA\Add_On\Spec2Vec\model_positive_mode_fast\020724_Spec2Vec_pos_embeddings.npy")
