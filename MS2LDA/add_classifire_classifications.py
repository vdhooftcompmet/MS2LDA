...
def select_inchikeys(spectra):
    ...
    for spectrum in spectra:
        list_of_inchikeys.append(spectrum.get("inchikey"))
...
def select_smiles_and_full_inchikeys(spectra):
    ...
    for spectrum in spectra:
        ...
        inchikey_dict[inchikey].append([inchikey, smiles])
    return inchikey_dict


def do_url_request(url: str) -> bytes or None:
    """
    ...
    try:
        ...
        print("A connection error occurred, will try again")
        result = do_url_request(url)  # Retry the request
    return result
...
def convert_to_dataframe(inchikey_results_lists)->pd.DataFrame:
    ...
    df.set_index("inchikey", inplace=False)
    return df
