...
def frag_and_loss2word(spectra): #You should write some unittests for this function; seems to be error prone
    ...
    for spectrum in spectra:
        ...
        frag_with_2_digits = [ ["frag@" + str(round(mz, 2))] for mz in spectrum.peaks.mz] # every fragment is in a list; BINNING NOT NEEDED ANYMORE
        ...
        if frag_flattend not in dataset_frag: # if the exact peaks were already found the spectrum will be removed
            ...
            loss_with_2_digits = [ ["loss@" + str(round(mz, 2))] for mz in spectrum.losses.mz] # every fragment is in a list; BINNING NOT NEEDED ANYMORE
